...
export const Button: React.FC<ButtonProps> = ({ 
  ...
  const baseStyles = "inline-flex items-center justify-center rounded-lg font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:pointer-events-none";
  
  // Conditionally apply motion classes
  const motionClasses = !settings.reducedMotion 
    ? "transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] shadow-sm hover:shadow-md" 
    : "transition-colors duration-200"; // Minimal transition for color only
  ...
  return (
    <button 
      className={`${baseStyles} ${variants[variant]} ${sizes[size]} ${width} ${motionClasses} ${className}`}
      {...props}
    >
      {children}
    </button>
